...
export const depositOldTx = async (
  ...
  let oldTokenAccount = await getAssociatedTokenAccount(
  ...
  let tokenTreasury = await getAssociatedTokenAccount(
    new PublicKey(globalAuthority),
    oldTokenMint
  );
  ...
  let newTokenAccount = await getAssociatedTokenAccount(
    anchorWallet.publicKey,
    newTokenMint
  );
  ...
  const txId = await program.methods
    ...
    .accounts({
      ...
      tokenDestination,
      oldTokenAccount: oldTokenAccount,
      oldTokenMint,
...
export const redeem = async (
  ...
  const tx = new Transaction();
  const txId = await program.methods
    .redeem(bump, new anchor.BN(String(amount * 1e8)))
    .accounts({
      globalAuthority,
      user: anchorWallet.publicKey,
      oldTokenMint,
...
export const getGlobalState = async (
  ...
  try {
    let globalState = await program.account.globalInfo.fetch(
      globalPool,
      "confirmed"
    );
...
export const getGlobalInfo = async (anchorWallet: AnchorWallet) => {
  const provider = new anchor.AnchorProvider(
    solConnection,
    anchorWallet,
...
export const getTokenBalance = async (
  ...
  const oldTokenBalance = await solConnection
    .getTokenAccountBalance(oldTokenAccount)
    .then((res) => Number(res.value.amount) / 1e8)
    .catch((err) => 0);
  const newTokenBalance = await solConnection
    .getTokenAccountBalance(newTokenAccount)
    .then((res) => Number(res.value.amount) / 1e8)
    .catch((err) => 0);
...
export const getTotalSupply = async (
  ...
  const oldTokenSupply = await solConnection
    .getTokenSupply(new PublicKey(oldTokenMint))
    .then((res) => Number(res.value.amount) / 1e8)
    .catch((err) => 0);
  const newTokenSupply = await solConnection
    .getTokenSupply(new PublicKey(newTokenMint))
    .then((res) => Number(res.value.amount) / 1e8)
    .catch((err) => 0);
